...
from functools import lru_cache  # ✅ NEW: Cache non-scrollable URL parsing
...
from app.core.exceptions import BrowserError, AuthenticationError, PathValidationError  # ✅ Typed exceptions
from app.core.path_security import PathSecurityValidator  # ✅ PHASE 1: Path security
...
class ScreenshotService:
    ...
    def __init__(self):
        ...
        # ✅ PHASE 1 (Path Security): Validated directory paths
        self.output_dir = PathSecurityValidator.ALLOWED_BASES["screenshots"]
        ...
        # ✅ PHASE 1 (Path Security): Validated session directory
        self.session_dir = PathSecurityValidator.ALLOWED_BASES["browser_sessions"]
        self.session_dir.mkdir(exist_ok=True, mode=0o755)
        ...
        # ✅ PHASE 1 (Path Security): Use validated browser session path
        self.cookies_file = self.session_dir / "cookies.json"
        self.cookies_file.parent.mkdir(exist_ok=True, mode=0o755)
    ...
    def _load_hash_cache(self) -> None:
        ...
        try:
            if self._hash_cache_file.exists():
                # ✅ PHASE 1 (Path Security): Validate before reading
                validated_path = PathSecurityValidator.validate_path(
                    path=self._hash_cache_file,
    ...
    def _parse_non_scrollable_patterns(non_scrollable_urls: str) -> tuple:
        """
        Parse non-scrollable URL patterns with LRU caching.

        ✅ PERFORMANCE OPTIMIZATION: Caches parsed patterns to avoid repeated JSON parsing.
        - Without cache: 50 URLs = 50 json.loads() calls
        - With cache: 50 URLs = 1 json.loads() + 49 cache hits (50x faster!)

        Returns:
            tuple: Immutable tuple of URL patterns (required for caching)

        Example:
            patterns = self._parse_non_scrollable_patterns('["#media-upload", "/parts/"]')
            # Returns: ('#media-upload', '/parts/')
        """
        if not non_scrollable_urls:
            return ()

        try:
            patterns = json.loads(non_scrollable_urls)
            return tuple(patterns)  # Convert to tuple for immutability (required by lru_cache)
        except Exception as e:
            logger.warning("Failed to parse non_scrollable_urls: %s", e)
            return ()
    ...
    def _save_hash_cache(self) -> None:
        ...
        """
        try:
            # ✅ PHASE 1 (Path Security): Validate before writing
            validated_path = PathSecurityValidator.validate_path(
                path=self._hash_cache_file,
                base_dir=self.output_dir,
                must_exist=False,
    ...
    async def _connect_to_chrome_cdp(self, cdp_url: str = "http://localhost:9223", max_retries: int = None):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.debug("=" * 60 + "\n")
                raise BrowserError(
                    reason="Cannot connect to Chrome. Please launch Chrome with remote debugging enabled. Run: ./launch-chrome-debug.sh",
                    browser_type="chrome"
                )
    ...
    async def _get_active_tab(self):
        ...
        if self.cdp_browser is None:
            raise BrowserError(
                reason="Not connected to Chrome via CDP. Call _connect_to_chrome_cdp() first.",
                browser_type="chrome"
            )
        ...
        if not contexts:
            raise BrowserError(
                reason="No browser contexts found. Make sure Chrome has at least one window open.",
                browser_type="chrome"
            )
        ...
        if not pages:
            raise BrowserError(
                reason="No tabs found in Chrome. Make sure Chrome has at least one tab open.",
                browser_type="chrome"
            )
        ...
        if active_page is None:
            raise BrowserError(
                reason="Could not find active tab.",
                browser_type="chrome"
            )
    ...
    async def _create_new_tab_next_to_active(self):
        ...
        try:
            ...
            raise BrowserError(
                reason="Failed to create or find a usable tab for screenshot capture",
                browser_type="chrome"
            )
    ...
    async def capture_segmented(
        ...
        try:
            ...
            # ✅ OPTIMIZED: Check if URL matches non-scrollable patterns (cached)
            non_scrollable_patterns = self._parse_non_scrollable_patterns(non_scrollable_urls)
    ...
    async def save_auth_state(self, url: str, storage_state_path: str, browser_engine: str = "playwright"):
        ...
        if elapsed >= max_wait_time:
            raise AuthenticationError(
                reason="Timeout waiting for login. Please try again.",
                url=url
            )
        ...
        try:
            ...
            raise AuthenticationError(
                reason=f"Failed to save auth state: {str(e)}",
                url=url
            )
