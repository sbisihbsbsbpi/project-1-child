...
def generate_edit_url(template_id):
    ...
    - 'templateId': Public template ID (e.g., 667f0befd4964026ee7b6e6e or CPRA_REQUEST_...)

    The edit URL uses templateId!

    Args:
        template_id: The templateId from the API response
    ...
    async with async_playwright() as playwright:
        try:
            ...
            if api_data['before'] and api_data['after']:
                ...
                # Validate: API 'hits' array count matches our count
                actual_after_count = len(after['templates'])
                ...
                if added:
                    print(f"   📝 Sample Added Templates (from API 'hits' array):")
                ...
                # Important note about id vs templateId
                print(f"   ⚠️  Important: Templates have TWO ID fields:")
                print(f"      'id':         Internal database ID (don't use for URLs)")
                print(f"      'templateId': Public template ID (use for edit URLs) ✅")
                print()

                # Emphasize that we have REAL DATA, not just counts
                print(f"   💡 Key Insight:")
