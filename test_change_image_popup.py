...
async def main():
    ...
    async with async_playwright() as playwright:
        ...
        logger.info("\n" + "=" * 100)
        logger.info("STEP 1: Finding ALL logos with warnings")
        logger.info("=" * 100)

        # Find ALL logos with warnings
        logos_info = await page.evaluate("""
            () => {
                const warningIcons = Array.from(document.querySelectorAll('.templates_Image_warningIcon__hCZHMuhEmb'));
                if (warningIcons.length === 0) return { found: false, count: 0 };

                warningIcons.forEach((icon, idx) => {
                ...
                return {
                    found: true,
                    count: warningIcons.length
                };
            }
        """)

        if not logos_info['found']:
            logger.error("❌ No logos with warning found")
            return

        logger.info(f"✅ Found {logos_info['count']} logo(s) with warning")

        # Process each logo
        for logo_idx in range(1, logos_info['count'] + 1):
            logger.info("\n" + "=" * 100)
            logger.info(f"🎯 PROCESSING LOGO #{logo_idx} of {logos_info['count']}")
            logger.info("=" * 100)
        ...
        logger.info("=" * 100)
        logger.info(f"\n✅ Processed {logos_info['count']} logo(s) successfully!")
        ...
        logger.info(f"   4. All {logos_info['count']} logo(s) should have new selections")
